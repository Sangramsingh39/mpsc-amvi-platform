...
import { Play, CheckCircle2, Award, Clock, BarChart3, Search, BookOpen } from 'lucide-react';
...
export default function TestDashboard({ tests, stats, onStartTest, onReviewAttempt, onOpenAdmin, lang, setLang }) {
  ...
  const filteredTests = tests.filter((t) => {
    const cleanTestTitle = (t.title || '').replace(/\s*Full\s*/i, ' ');
    const matchesSearch = cleanTestTitle.toLowerCase().includes(search.toLowerCase()) || `Test ${t.id}`.toLowerCase().includes(search.toLowerCase());
          ...
          return (
            ...
            >
              <div>
                ...
                <h3 className="text-base font-bold text-dark mb-2">{cleanTitle}</h3>
                ...
                <div className="text-xs text-dark-secondary space-y-1 mb-4">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span>Duration: {test.duration_minutes || 45} Minutes</span>
                  </div>
                </div>
              </div>
